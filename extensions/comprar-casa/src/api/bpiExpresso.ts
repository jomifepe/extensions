...
export const fetchBpiExpressoListings = async (options?: ApiFetcherOptions): Promise<PaginatedData<Listing>> => {
  ...
  const result = await fetch(
    `https://api.bpiexpressoimobiliario.pt/api/v1/announces/?type=buy&kinds=apartment&kinds=house&district=Leiria&county=Leiria&range_value=price&range_value_from=99000&range_value_to=201000&typologies=t2&typologies=t3&typologies=t4&typologies=t5&typologies=t5_more&all_selected_characteristics=true&days_inserted=all&is_paa=false&only_bpi_properties=false&only_partner_properties=false&only_particular_properties=false&page=${page}&page_size=20&sort=latest`,
    {
      headers: {
